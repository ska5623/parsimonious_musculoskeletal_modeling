...
%           filestr = ['dataasync_thorax_' num2str(MT) '_' num2str(P1all(p1i)) '.mat'];
%           save(filestr,'disp','muscleforce_d','muscleforce_u','K1','K2'); 
%        end
% end


%% thorax work loop map plotting
P1all = [2 4 6 14 31];
...
for mti = 1:1:5
       for p1i = 1:1:5
             P1 = P1allnew(P1all(p1i));
             MT = MTall(1,mti);
             filestr = ['Thorax data async/dataasync_thorax_' num2str(MT) '_' num2str(P1all(p1i))  '.mat'];
             load(filestr);
             K1all(p1i,mti) = K1;
             K2all(p1i,mti) = K2;
